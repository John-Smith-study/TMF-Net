...
class PromptEncoder(nn.Module):
    ...
    def _embed_masks(self, masks: torch.Tensor) -> torch.Tensor:
        ...
        # 确保输入是单通道的
        if masks.size(1) > 1:
            # 如果是多通道输入（如4通道），转换为单通道
            # 可以取均值或只使用第一个通道
            masks = masks.mean(dim=1, keepdim=True)  # 取通道均值
...
class PositionEmbeddingRandom(nn.Module):
    ...
    def _pe_encoding(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        # 【关键修复】确保coords与positional_encoding_gaussian_matrix在同一设备上
        device = self.positional_encoding_gaussian_matrix.device
        coords = coords.to(device) @ self.positional_encoding_gaussian_matrix.to(torch.float32) # todo
