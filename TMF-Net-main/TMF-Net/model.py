...
class QualityGateStrategy:
    """
    预测质量 + 质量改善趋势 双条件门控策略 (按照论文描述实现)
    
    公式 (10) - 质量变化动量 (TQM):
        TQM_t = β * TQM_{t-1} + (1 - β) * (q_t - q_{t-1})
    其中:
        - q_t: 第 t 轮模型预测的 mask quality
        - β = 0.5
        - q_0 = 0, TQM_0 = 0
        - TQM_t > 0 表示预测质量近期仍在改善
    ...
    公式 (13) - 记忆缓冲区更新:
        B_t = { Append8(B_{t-1}, h̃_t),  g_t = 1
              { B_{t-1},                g_t = 0
    缓冲区最多保留 8 个状态。若 g_t = 0，当前预测仍然有效，只是不更新 GRU 状态和记忆缓冲区。
    
    训练时阈值线性预热:
        τ_q(e) = τ_q_max * (e - E_start) / (E_end - E_start),  E_start ≤ e < E_end
        τ_tqm(e) = τ_tqm_max * (e - E_start) / (E_end - E_start),  E_start ≤ e < E_end
    
    论文设定:
        E_start = 50, E_end = 120, τ_q_max = 0.75, τ_tqm_max = 0.02
    
    推理时直接使用最终阈值:
        g_t = I(q_t > 0.75 ∧ TQM_t > 0.02)
    """
    
    def __init__(self, max_q_threshold=0.75, max_tqm_threshold=0.02,
                 warmup_start=50, warmup_end=120, beta=0.5):
        """
        初始化质量门控策略
        
        Args:
            max_q_threshold: τ_q_max, 最大质量阈值，默认 0.75
            max_tqm_threshold: τ_tqm_max, 最大TQM阈值，默认 0.02
            warmup_start: E_start, 预热起始epoch，默认 50
            warmup_end: E_end, 预热结束epoch，默认 120
            beta: β, TQM动量衰减系数，默认 0.5
        """
        self.max_q_threshold = max_q_threshold
        self.max_tqm_threshold = max_tqm_threshold
        self.warmup_start = warmup_start
        self.warmup_end = warmup_end
        self.beta = beta
    
    def _get_dynamic_thresholds(self, global_epoch, is_training=True):
        """
        根据当前 epoch 动态计算阈值
        ...
        if epoch < self.warmup_start:
            ...
            q_threshold = self.max_q_threshold * progress
            tqm_threshold = self.max_tqm_threshold * progress
            return q_threshold, tqm_threshold, False
        else:
            # 稳定期：使用最大阈值
            return self.max_q_threshold, self.max_tqm_threshold, False
    
    def calculate_tqm(self, q_t, q_prev, tqm_prev):
        """
        计算时序质量动量 (TQM)
        ...
        Args:
            q_t: 当前轮次的预测质量 q_t
            q_prev: 上一轮的预测质量 q_{t-1}
            tqm_prev: 上一轮的 TQM 值 TQM_{t-1}
        ...
        tqm_t = self.beta * tqm_prev + (1 - self.beta) * (q_t - q_prev)
        return tqm_t
    
    def check_quality(self, q_t, q_prev, tqm_prev, global_epoch=0, is_training=True):
        """
        ...
        Returns:
            tuple: (g_t, q_t, tqm_t, q_threshold, tqm_threshold, is_warmup)
                   (门控决策, 当前质量, 当前TQM, 质量阈值, TQM阈值, 是否预热期)
        """
...
class TemporalMemoryModule(nn.Module):
    ...
    def __init__(self, max_history_length=8, feature_dim=256):
        ...
        # TQM 状态追踪: tqm_state[patient_id_str][category_str] = {'q_prev': float, 'tqm_prev': float}
        self.tqm_state = {}
        
        self.quality_gate = QualityGateStrategy(
            max_q_threshold=0.75,
            max_tqm_threshold=0.02,
            warmup_start=50,
            warmup_end=120,
            beta=0.5
        )
    ...
    def _safe_convert_category(self, category):
        ...
        if isinstance(category, torch.Tensor):
            ...
            return str(category)
        return category
    def store(self, patient_id, category, data, global_epoch=0, is_training=True): 
        """按类别存储病人历史记录 - TQM双条件门控版
        ...
        try:
            ...
            # 获取 TQM 状态 (q_{t-1}, TQM_{t-1})
            if patient_id_str not in self.tqm_state:
                self.tqm_state[patient_id_str] = {}
            if category_str not in self.tqm_state[patient_id_str]:
                self.tqm_state[patient_id_str][category_str] = {'q_prev': 0.0, 'tqm_prev': 0.0}
            
            tqm_entry = self.tqm_state[patient_id_str][category_str]
            q_prev = tqm_entry['q_prev']
            tqm_prev = tqm_entry['tqm_prev']
            
            # 门控决策: g_t = I(q_t > τ_q(e) ∧ TQM_t > τ_tqm(e))
            g_t, q_t, tqm_t, q_threshold, tqm_threshold, is_warmup = (
                self.quality_gate.check_quality(q_t, q_prev, tqm_prev, global_epoch, is_training)
            )
            
            stage = "WARMUP" if is_warmup else ("RAMP" if global_epoch < self.quality_gate.warmup_end else "STABLE")
    ...
    def update_history(self, patient_id, data, global_epoch=0, is_training=True):
        ...
        self.store(patient_id, category, data, global_epoch, is_training)
    ...
    def clear_patient_history(self, patient_id):
        ...
        if patient_id_str in self.tqm_state:
            del self.tqm_state[patient_id_str]
            model_logger.debug(f"[INFO] Cleared TQM state for patient {patient_id_str}")
    ...
    def clear_all_history(self):
        ...
        self.tqm_state.clear()
        model_logger.debug("[INFO] Cleared all TQM state")
        ...
        if category is not None:
            ...
            if patient_id_str in self.history_buffer and category_str in self.history_buffer[patient_id_str]:
                ...
                if not isinstance(self.history_buffer[patient_id_str], dict) or len(self.history_buffer[patient_id_str]) == 0:
                    del self.history_buffer[patient_id_str]
            # sync TQM state for this (patient, category) pair
            if patient_id_str in self.tqm_state and category_str in self.tqm_state[patient_id_str]:
                del self.tqm_state[patient_id_str][category_str]
                if not isinstance(self.tqm_state[patient_id_str], dict) or len(self.tqm_state[patient_id_str]) == 0:
                    del self.tqm_state[patient_id_str]
        else:
            if patient_id_str in self.history_buffer:
                del self.history_buffer[patient_id_str]
...
class LSTMTemporalModule(nn.Module):
    ...
    def construct_interaction_sequence(self, interactions, image_size=(256, 256)):
        ...
        # Eq.(12): collect per-record g_t (default 1 since g_t=0 never appended)
        gated_values = []
        for interaction in interactions:
            if isinstance(interaction, dict):
                g_raw = interaction.get("gated", interaction.get("g_t", 1))
                try:
                    gated_values.append(float(g_raw) if isinstance(g_raw, (int, float)) else 1.0)
                except Exception:
                    gated_values.append(1.0)
            else:
                gated_values.append(1.0)
        ...
        return sequence_tensor, seq_lengths, gated_values
    def extract_trajectory_features(self, interaction_sequence, patient_id=None, gated_values=None):
        """Eq.(12) gated GRU hidden state for interaction trajectory feature extraction.

        H_t = g_t * H_tilde_t + (1 - g_t) * H_{t-1}
        g_t comes from each stored history record's 'gated' field (default 1 since g_t=0 never stored).
        """
        if gated_values is None:
            gated_values = [1.0]
        device = next(self.parameters()).device
        self.device = device
        ...
        if isinstance(interaction_sequence, torch.Tensor):
            ...
            sequence_tensor, seq_lengths, _ = self.construct_interaction_sequence(interaction_sequence)
        ...
        h_prev = torch.zeros(2, batch_size, self.feature_dim, device=device)
        # Align gated_values length to actual sequence length
        seq_len_actual = sequence_tensor.shape[1]
        if not isinstance(gated_values, (list, tuple)):
            gated_values = [1.0]
        gated_list = list(gated_values)
        if len(gated_list) < seq_len_actual:
            gated_list = gated_list + [1.0] * (seq_len_actual - len(gated_list))
        elif len(gated_list) > seq_len_actual:
            gated_list = gated_list[:seq_len_actual]
        
        try:
            if not isinstance(sequence_tensor, torch.Tensor):
                sequence_tensor = torch.zeros(batch_size, 1, self.feature_dim, device=device)
            
            # Eq.(12): step-wise hidden-state gating H_t = g_t * H_tilde_t + (1 - g_t) * H_{t-1}
            temporal_out_steps = []
            h_curr = h_prev
            for t in range(seq_len_actual):
                x_t = sequence_tensor[:, t:t + 1, :]
                _, h_tilde = self.temporal_memory(x_t, h_curr)
                g_t = float(gated_list[t])
                h_curr = g_t * h_tilde + (1.0 - g_t) * h_curr
                # top-layer hidden state as step output: [1, B, D] -> [B, 1, D]
                temporal_out_steps.append(h_curr[-1:, :, :].transpose(0, 1).contiguous())
            if temporal_out_steps:
                temporal_out = torch.cat(temporal_out_steps, dim=1)
            else:
                temporal_out = torch.zeros(batch_size, 0, self.feature_dim, device=device)
...
class MultiScaleTemporalFusion(nn.Module):
    ...
    def __init__(self, image_size=256, feature_dim=768, num_scales=4, 
        ...
        self._pending_store = []  # (patient_id, category, data_dict, epoch, is_training)
    def get_temporal_context(self, patient_id, category=None): 
        ...
        sequence_tensor, _, gated_values = self.trajectory_analyzer.construct_interaction_sequence(interaction_sequences)
        if not isinstance(gated_values, (list, tuple)):
            gated_values = [1.0] 
        ...
        return (sequence_tensor.to(next(self.parameters()).device), gated_values)
    @staticmethod
    ...
    def model_temporal_dependencies(self, patient_id, category, current_prompt): 
        ...
        try:
            ...
            return current_prompt, None
    def update_interaction_history(self, patient_id, current_data, global_epoch=0, is_training=True):
        """更新历史记录 - TQM门控版：支持传入epoch和训练模式"""
        self.temporal_memory.update_history(patient_id, current_data, global_epoch, is_training)
    ...
    def forward(self, current_prompt, history, image_features, is_training=True):
        """前向传播 - 优化版（支持消融实验）"""
        ...
        quality_score_val = _safe_scalar(
            current_prompt.get('quality_score',
                current_prompt.get('iou_predictions',
                    current_prompt.get('iou_pred', dice_val))))
        temporal_data = {
            ...
            'iou_predictions': quality_score_val,
            'category': category,
            'epoch': global_epoch,
        ...
        # Deferred: real store happens after decoder outputs iou_pred (= paper q_t) in IMIS.forward_with_features
        self._pending_store.append((patient_id, category, dict(temporal_data), global_epoch, is_training))
        ...
        if not self.ablation_no_trajectory and hasattr(self, 'trajectory_analyzer'):
            try:
                ctx = self.get_temporal_context(patient_id, category)
                if isinstance(ctx, tuple) and len(ctx) == 2:
                    sequence_tensor, gated_values = ctx
                else:
                    sequence_tensor = ctx
                    gated_values = [1.0]
                if sequence_tensor is not None and isinstance(sequence_tensor, torch.Tensor):
                    ...
                    if seq_len >= 1:
                        sequence_tensor = sequence_tensor.to(device)
                        trajectory_feature = self.trajectory_analyzer.extract_trajectory_features(
                            sequence_tensor, patient_id=patient_id, gated_values=gated_values) 
                    else:
...
class TMFNet(IMISNet):
    ...
    def check_quality_gate(self, q_t, q_prev, tqm_prev, global_epoch=0, is_training=True):
        """质量门控检查 - 委托给 quality_gate 策略"""
        return self.temporal_memory.quality_gate.check_quality(q_t, q_prev, tqm_prev, global_epoch, is_training)
