...
class TemporalMemoryModule(nn.Module):
    ...
    def store(self, patient_id, category, data, global_epoch=0): 
        ...
        try:
            ...
            # Use model-estimated quality_score (e.g. SAM iou_predictions) for the gate.
            # Fall back to dice only during training; never use GT-derived dice at test time.
            quality = ultra_safe_scalar(
                data.get('quality_score', data.get('iou_predictions', data.get('dice', 0.0)))
            )
            dice = ultra_safe_scalar(data.get('dice', 0.0))  # debug/log only, not used for gate
            global_epoch = ultra_safe_scalar(global_epoch)
            print(f"[STOREDEBUG] converted values: quality(gate)={quality:.4f}, dice(log)={dice:.4f}, epoch={global_epoch}")
            
            # Build quality history for TCI computation (uses the actual gate metric, not dice)
            ...
            # Quality gate check (Eq. 4) with dynamic thresholds
            ...
            reduced_data = {
                'quality': quality,     # model-estimated quality, used for TCI history (not GT-derived)
                'dice': dice,           # debug/log only, not used for memory gate
                'tci': current_tci,
                'epoch': global_epoch,
            ...
            # Quality gate (Eq. 4): only store qualified records into memory buffer
            # B_t = { {Z_t, h_t}, if q_t > tau_q and TCI_t > tau_tci
            #       { B_{t-1}, otherwise
            if not is_qualified:
        ...
        if isinstance(history_list, (list, tuple)) and len(history_list) > 0:
            ...
            for i, hist in enumerate(reversed(recent_history)):
                if not isinstance(hist, dict):
                    continue
                # Use model-estimated quality (not GT-dice) for history weighting;
                # fall back to dice only when quality is missing (training-time compat)
                hist_quality = hist.get('quality', hist.get('dice', 0.0))
                hist_tci = hist.get('tci', 0.0)
                # quality_weight: quality 0.7 + TCI 0.3
...
class TMFNet(IMISNet):
    ...
    def postprocess_difficult_samples(self, pred_mask, label_mask=None, quality_score=None):
        ...
        # --- Step 2: threshold (quality-based or fixed 0.5; no label used) ---
        ...
        # --- Step 3a: morphological cleanup (open+close, 3x3, label-free) ---
        try:
            k3 = torch.ones((1, 1, 3, 3), device=device)
            work = binary_mask
            while work.ndim < 4:
                work = work.unsqueeze(0)
            # open: erode -> dilate (remove small noise)
            ...
            # close: dilate -> erode (connect small gaps)
        ...
        # --- Step 3b: remove connected components smaller than k=20 pixels (label-free) ---
        try:
