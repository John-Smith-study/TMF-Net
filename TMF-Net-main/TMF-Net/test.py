                    ...
                    try:
                        ...
                        try:
                            ...
                            if temp_dice < diff_thresh and is_valid_gt:  # 只处理有效GT的困难样本
                                ...
                                try:
                                    # Unified 8-click budget (consistent with paper)
                                    # BTCV no longer gets a special 15-round budget; all datasets use self.args.inter_num
                                    print(
                                        f"[INFO] Batch {step + 1}: Class {cls_idx + 1}: Difficult sample detected: Dice={temp_dice:.4f}, enabling unified {self.args.inter_num}-round interaction (fixed budget)")

                                    # No BTCV-specific stage_factor override; keep the model's original schedule
                                    cls_round_dice = [temp_dice]
                                    current_mask = mask_preds.clone()
                                    current_low_masks = None

                                    # Unified self.args.inter_num (should be 8 for main experiments)
                                    for inter_round in range(self.args.inter_num):
                                        loss, current_mask, current_low_masks, round_dice = self.interaction_single_round(
                                            model, image_embedding, current_low_masks, current_mask, interaction_labels,
                                        ...
                                        # Early stop for difficult samples: stop if 3 consecutive rounds improve < 0.002
                                        # (does not exceed the budget cap, only saves inference time)
                                        if inter_round > 3 and all(
                                            cls_round_dice[-i] - cls_round_dice[-i-1] < 0.002
                                            for i in range(1, 4)
                                        ):
                                            print(f"[EARLY_STOP] Batch {step + 1}: Class {cls_idx + 1}: Difficult sample converged at round {inter_round+1}")
                                            break

                                    processed_mask = current_mask.clone()

                                    # Compliant post-processing pipeline - only allows:
                                    #   1) fixed/quality-based threshold binarization (no GT scan)
                                    #   2) morphological cleanup (remove small CCs < k=20 pixels)
                                    #   3) BTCV: keep largest connected component (label-free, bottom-up)
                                    # Forbidden: GT-based threshold scan, GT-area CC selection, GT-scored candidate masks
                                    print(
                                        f"[DEBUG] Batch {step + 1}: Class {cls_idx + 1}: Applying compliant post-processing (threshold+cleanup)")

                                    # 1) Binarization (fixed 0.5 threshold; quality_score not passed -> unified 0.5)
                                    processed_mask = self._adaptive_threshold(processed_mask, labels=None)

                                    # 2) Morphological cleanup (all datasets, k~20)
                                    processed_mask = self._morphological_cleanup(processed_mask)

                                    # 3) BTCV largest connected component (label-free, kept; other datasets skip)
                                    if self.args.dataset.lower() == 'btcv' and hasattr(self, '_keep_largest_connected_component'):
                                        print(f"[DEBUG] Batch {step + 1}: Class {cls_idx + 1}: BTCV keep largest CC")
                                        processed_mask = self._keep_largest_connected_component(processed_mask)

                                    # Removed:
                                    # - _adaptive_threshold(processed_mask, labels_cls) -> no GT label passed
                                    # - model.postprocess_difficult_samples(processed_mask, adjusted_labels) -> depends on GT area and GT Dice scoring, disabled in main experiments

                                except Exception as e:
                                    print(
                                        f"[WARNING] Batch {step + 1}: Class {cls_idx + 1}: Difficult-sample processing error: {e}, using original mask_preds")
                                    processed_mask = mask_preds.clone()
                                    # Fallback: basic fixed-threshold binarization
